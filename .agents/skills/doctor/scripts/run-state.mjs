...
async function main() {
    ...
    const previous =
        options.action === 'start' ? null : await readState(projectRoot);
    const state = buildState(options, previous, new Date().toISOString());
    await writeState(projectRoot, state);
    console.log(
        `Recorded /${state.command}: ${state.status} - ${state.summary}`,
    );
}
...
function validateOptions(options) {
    ...
    if (!options.action) {
        throw new Error(
            'Choose one run-state action: start, update, finish, or reset.',
        );
    }
    ...
    if (options.action === 'start') {
        if (!options.command || !options.summary || !options.boundary) {
            throw new Error(
                'Start requires --command, --summary, and --boundary.',
            );
        }
        if (options.status) {
    ...
    if (options.action === 'finish') {
        if (
            options.status &&
            !['ready', 'completed'].includes(options.status)
        ) {
    ...
    if (
        options.status &&
        !['running', 'blocked', 'ready'].includes(options.status)
    ) {
...
function validateState(value) {
    ...
    if (
        ...
        (value.detail !== undefined &&
            !isBoundedText(value.detail, MAX_LENGTHS.detail)) ||
    ...
    while (true) {
        ...
        try {
            ...
            if (state.isSymbolicLink() || !state.isDirectory()) {
                throw new Error(
                    'Blueprint state path must be a real directory.',
                );
            }
            return current;
        } catch (error) {
        ...
        if (parent === current) {
            throw new Error(
                'Could not find a Blueprint project with blueprint/.state.',
            );
        }
        current = parent;
    }
...
async function readState(projectRoot) {
    ...
    try {
        ...
        throw new Error(
            'Existing dashboard state is malformed. Start a new run or use /doctor.',
        );
    }
