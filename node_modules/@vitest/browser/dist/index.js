...
import { isFileLoadingAllowed, isValidApiRequest, isFileServingAllowed, distDir, rolldownVersion, resolveApiServerConfig, resolveFsAllow, createDebugger, createViteLogger, createViteServer } from 'vitest/node';
...
function assertBrowserFileAccess(project, path) {
	const normalized = slash(path);
	if (!isFileLoadingAllowed(project.vite.config, normalized) && !isFileLoadingAllowed(project.vitest.vite.config, normalized)) {
		throw new Error(`Access denied to "${path}". See Vite config documentation for "server.fs": https://vitejs.dev/config/server-options.html#server-fs-strict.`);
	}
}
function assertBrowserApiWrite(project, path) {
	if (!project.config.browser.api.allowWrite || !project.vitest.config.api.allowWrite) {
		throw new Error(`Cannot modify file "${path}". File writing is disabled because the server is exposed to the internet, see https://vitest.dev/config/browser/api.`);
	}
}
...
async function performSideEffects(outcome, codec, project) {
	switch (outcome.type) {
		...
		case "update-reference": {
			await writeScreenshot(outcome.reference.path, await codec.encode(outcome.reference.image, {}), project);
			break;
		}
		case "mismatch": {
			await writeScreenshot(outcome.actual.path, await codec.encode(outcome.actual.image, {}), project);
			if (outcome.diff) {
		...
		const rpc = createBirpc({
			...
			async onTaskArtifactRecord(id, artifact) {
				if (!canWrite(project)) {
					...
					const attachments = artifact.type === "internal:annotation" ? artifact.annotation.attachment ? [artifact.annotation.attachment] : [] : artifact.attachments ?? [];
...
export { asLocator, assertBrowserApiWrite, assertBrowserFileAccess, createBrowserServer, defineBrowserCommand, defineBrowserProvider, parseKeyDef, resolveScreenshotPath };
