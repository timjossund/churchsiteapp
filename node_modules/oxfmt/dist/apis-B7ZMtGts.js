...
async function setupTailwindPlugin(options) {
	CACHES.tailwindPlugin ??= await loadCached("tailwindPlugin", () => import("./dist-Bvl1HZN6.js"));
	options.plugins ??= [];
...
async function sortTailwindClasses({ classes, options }) {
	CACHES.tailwindSorter ??= await loadCached("tailwindSorter", () => import("./sorter-NRfyh-Pj.js"));
...
async function setupSveltePlugin(options) {
	CACHES.sveltePlugin ??= await loadCached("sveltePlugin", async () => await import("./plugin-B2qgoFkH.js").then((m) => /* @__PURE__ */ __toESM(m.default, 1)));
	options.plugins ??= [];
