	...
	if (process.env.NAPI_RS_NATIVE_LIBRARY_PATH) try {
		...
		if (process.arch === "arm64") {
			...
			try {
				...
				if (bindingPackageVersion !== "0.64.0" && process.env.NAPI_RS_ENFORCE_VERSION_CHECK && process.env.NAPI_RS_ENFORCE_VERSION_CHECK !== "0") throw new Error(`Native binding package version mismatch, expected 0.64.0 but got ${bindingPackageVersion}. You can reinstall dependencies to fix this issue.`);
				return binding;
			} catch (e) {
				loadErrors.push(e);
			}
		...
		try {
			...
			if (!candidateFailed) {
				if (process.env.NAPI_RS_ENFORCE_VERSION_CHECK && process.env.NAPI_RS_ENFORCE_VERSION_CHECK !== "0") {
					...
					if (bindingPackageVersion !== "0.64.0") throw new Error(`WASI binding package version mismatch, expected 0.64.0 but got ${bindingPackageVersion}. You can reinstall dependencies to fix this issue.`);
				}
