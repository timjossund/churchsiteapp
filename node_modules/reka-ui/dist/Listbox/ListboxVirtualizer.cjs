...
var ListboxVirtualizer_vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ (0, vue.defineComponent)({
	...
	setup(__props) {
		...
		const virtualizedItems = (0, vue.computed)(() => virtualizer.value.getVirtualItems().map((item) => {
			...
			const targetNode = defaultNode.type === vue.Fragment && Array.isArray(defaultNode.children) ? defaultNode.children.find((child) => typeof child.type !== "symbol") : defaultNode;
			return {
				item,
		...
		function handleMultipleReplace(event, intent) {
			...
			switch (intent) {
				...
				case "last": {
					value = require_shared_arrays.findValuesBetween(props.options, rootContext.firstValue.value, props.options.at(-1));
					break;
				}
			}
