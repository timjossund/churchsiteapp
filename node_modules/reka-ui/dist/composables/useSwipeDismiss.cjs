...
function useSwipeDismiss(options) {
	...
	const drawerAxis = (0, vue.computed)(() => {
	...
	*/
	...
	function processMove(el, pos, time) {
		...
		if (pendingSwipe && pendingSwipeStartPos) {
			...
			if (pending < MIN_DRAG_THRESHOLD) return;
			pendingSwipe = false;
			setSwiping(true);
			onSwipeStart?.();
		}
		...
		if (!intendedDirection && dir) {
			...
			maxDisplacement = require_Drawer_utils.getDisplacement(dir, dx, dy);
		}
		...
		const progressDir = currentDir ?? (0, vue.toValue)(directions)[0];
		if (currentEl && progressDir) {
			...
			const p = Math.min(1, Math.max(0, require_Drawer_utils.getDisplacement(progressDir, dx, dy) / (dim + threshold)));
			if (p !== swipeProgress) {
				swipeProgress = p;
				onProgress?.(p, {
					deltaX: damped.x,
