...
function syncTimeSegmentValues(props) {
	return Object.fromEntries(require_date_parts.TIME_SEGMENT_PARTS.map((part) => {
		if (part === "dayPeriod") {
			const timeZone = require_date_comparators.isZonedDateTime(props.value) ? props.value.timeZone : void 0;
			return [part, props.formatter.dayPeriod(require_date_comparators.toDate(props.value), timeZone)];
		}
